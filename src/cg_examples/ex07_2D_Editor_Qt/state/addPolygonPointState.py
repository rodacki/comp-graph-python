...
class AddPointPolygonState(State):
    ...
    def mousePressEvent(self, event: QMouseEvent):
        if event.button() == Qt.MouseButton.LeftButton and self.context.global_vars.poligono:
            wx, wy = getWorldCoords(self.context, event.x(), event.y())
            p = Ponto(wx, wy)
            self.context.global_vars.poligono.addPonto(p)
            self.context.canvas.update()
            print(f"🔹 Ponto adicionado: ({wx:.2f}, {wy:.2f})")
    ...
    def mouseDoubleClickEvent(self, event):
        ...
        self.context.currentState = self.context.idleState
        self.context.canvas.update()
        print("🔴 Polígono finalizado com duplo clique")
